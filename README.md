...
```text
/hacsfiles/hass-perfectdraft-taproom-card/perfectdraft-taproom-card.js?v=4
```

The trailing `v=4` is intentional. Increment it when updating the card if Home Assistant or the browser keeps serving an older cached copy.
...
```yaml
url: /local/perfectdraft-taproom-card.js?v=4
type: module
```
...
- The `?v=4` query string shown in the resource URL examples.
